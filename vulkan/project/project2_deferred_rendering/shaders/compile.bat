..\..\glslc.exe gbuffer.vert -o gbuffer_vert.spv
..\..\glslc.exe gbuffer.frag -o gbuffer_frag.spv
..\..\glslc.exe full_quad.vert -o full_quad_vert.spv
..\..\glslc.exe full_quad_normal.frag -o full_quad_normal_frag.spv
..\..\glslc.exe full_quad_complex.frag -o full_quad_complex_frag.spv
..\..\glslc.exe ssao.frag -o ssao_frag.spv
..\..\glslc.exe ssao_blur.frag -o ssao_blur_frag.spv 
..\..\glslc.exe forward.vert -o forward_vert.spv
..\..\glslc.exe forward.frag -o forward_frag.spv
..\..\glslc.exe skybox.vert -o skybox_vert.spv
..\..\glslc.exe skybox.frag -o skybox_frag.spv
pause